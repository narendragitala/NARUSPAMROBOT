  ...
  if event.sender_id in SUDO_USERS:
     if event.reply_to_msg_id is not None:
        ...
        if int(user_id) in ALTRON:
            await event.reply("αвє נнαт кє вααℓ ує тєʀα внι вααρ нαι :: мтℓв ɴᴀʀᴜ вαℓα нαι", parse_mode=None, link_preview=None)
        elif int(user_id) == OWNER_ID:
            await event.reply("αвє ѕυи ує тєʀα внι вααρ нαι", parse_mode=None, link_preview=None)
        elif int(user_id) in SUDO_USERS:
            await event.reply("ѕυи иα внαι ℓαgтα нαι ує ɴᴀʀᴜ вαℓα нαι 🥰🥰", parse_mode=None, link_preview=None)
        else:
